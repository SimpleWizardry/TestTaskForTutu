  ...
  let showPage = (function() {
    ...
    return function(item) {
      ...
      function sortBy(event) {
        ...
        */
        sortedNotes = currentNotes.sort((prev, next) => {
          if (title == 'id') 
          {
            return prev[title] - next[title]
          }
      ...
      table.addEventListener('click', function() {
        ...
        btnSearch.addEventListener('click', function() {
          ...
          else if (!subStr && currentNotes == notes) 
          {
            console.log('не тыкай')
          }
          else 
          {
            notes.forEach(function(user, index, users) {
              for (key in user) {
      ...
      function renderTable(arr, sortedTitle = '', howSorted = '') {
        ...
        */
        ...
        for (let userNote of userNotes) {
          ...
          index = userNotes.indexOf(userNote);
          createCell(userNote.id, tr, index);
          createCell(userNote.firstName, tr, index);
          createCell(userNote.lastName, tr, index);
          createCell(userNote.email, tr, index);
          createCell(userNote.phone, tr, index);


          /* ПО УСЛОВИЮ ЭТИ СТОЛБЦЫ НЕ НУЖНЫ */
          createCell(userNote.adress.streetAddress, tr, index);
          createCell(userNote.adress.city, tr, index);
          createCell(userNote.adress.state, tr, index);
          createCell(userNote.adress.zip, tr, index);
          createCell(userNote.description, tr, index);
